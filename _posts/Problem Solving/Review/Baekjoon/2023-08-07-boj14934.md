...
그래서 1만칸짜리 배열을 만들어서, 한 칸에 하나의 자릿수만 가지도록 하였다. `arr[i] = 10^i의 자릿수`를 나타내도록 말이다. 이후에는 $i$를 1씩 늘려주면서 $c$의 값을 찾고, 그에 따른 ${a_n}^2$을 업데이트해서 가지고 있을 수 있도록 다. ${a_{n-1}}^2$의 값은 이미 가지고 있으므로, $2ca_{n-1} \times{10^l}$의 값과 $c^210^{2l}$값을 계산해서 넣어주면 된다.
