...
- 구하려는 해답은 만들어진 모든 벡터의 합의 최솟값을 요구한다.
  ...
  - 먼저 선택한 점의 집합을 $A$, 이후에 선택한 점의 집합을 $B$라고 할 때, 구하려는 해답은 $\min{|\sum_{i=1}^{N/2} A_i - B_i|}$로 나타낼 수 있다.
  - 위 수식은 $\min{|\sum_{i=1}^{N/2} A_i - \sum_{i=1}^{N/2} B_i|}$로도 나타낼 수 있다. 이 수식이 구현 상 더 쉬워서 활용하려한다.
