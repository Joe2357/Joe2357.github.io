...
바로 [페르마의 소정리](https://en.wikipedia.org/wiki/Fermat%27s_little_theorem)이다. 간단히 얘기하면, 어떤 소수 $p$에 대하여 $a^{p-1} \equiv 1 \mod p$를 만족한다는 정리이다. 그래서, $d$의 값이 매우 커지더라도, 우리는 $a^e \equiv a^{(e \% (p-1))} \pmod p$이라고 계산할 수 있다는 것이다. 문제에서 주어지는 모듈러의 값이 1백만 이하이므로, 우리가 고려해야할 $d$의 값은 커져봤자 1백만이 될 것이다. 문제의 단위를 매우 축소시킬 수 있다.
