...
- 조밀한 층으로 구성된 단순한 DNN과 BN
  ...
  - $\Delta \mu _i$와 $\Delta \sigma _i^2$가 크다면, BN-patch layer 이후의 neuron의 output destribution $\mathrm{BN}(\hat{\bar{x}}_i)$은 $\mathrm{BN}(\hat{x}_i)$와는 상당히 다른 값을 가질 것
...
### 3. Trainign and Testing Results Using MTFL
