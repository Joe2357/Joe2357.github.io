...
물론 우리는 이미 방법을 하나 알고 있다. 바로 [분할정복 알고리즘](https://joe2357.github.io/posts/Divide-and-Conquer/)을 활용한 거듭제곱 연산. 제곱하는 횟수 $X$를 절반씩 줄여나가기 때문에 빠른 시간 내에 거듭제곱 연산을 완료할 수 있다. $O(N \times \text{곱셈 연산})$이 들던 기존의 방법에서 $O(\log N \times \text{곱셈 연산})$으로 복잡도를 줄일 수 있다.
