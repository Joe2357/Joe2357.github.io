...
그룹 $X$에 있는 모든 정점에 대하여 [DFS 탐색](https://joe2357.github.io/posts/Graph-Traversal/#dfs-depth-first-search-깊이-우선-탐색/)을 이용하여 구현한다. 그룹 $X$의 노드는 그룹 $Y$의 노드 중 하나를 점유하려고 시도한다. 이 때 아래와 같은 선택지에 마주한다. 아래에서는 그룹 $X$의 노드를 $x$, 그룹 $Y$의 노드 중 현재 점유하려고 시도 중인 노드를 $y$라고 하겠다.
